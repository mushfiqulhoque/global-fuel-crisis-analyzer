...
# ─────────────────────────────────────────────────────────────────────────────
# EXACT column names as generated by preprocessing.py
# ─────────────────────────────────────────────────────────────────────────────

FEATURES = [
    # Brent lags — primary signal (shift(N) in preprocessing)
    "brent_crude_lag1m",
    "brent_crude_lag2m",
    "brent_crude_lag3m",
    "brent_crude_lag6m",
    "brent_crude_lag12m",
    # Brent rolling — shift(1)+rolling in preprocessing
    ...
    "brent_crude_pct1m",
    "brent_crude_pct3m",
    # Brent momentum features
    "brent_crude_momentum_3_12",
    "brent_crude_range_position",
    "brent_crude_vol_regime",
    # WTI lags — correlated market signal
    "wti_crude_lag1m",
    "wti_crude_lag2m",
    "wti_crude_lag3m",
    # Spread (shift(1) based in preprocessing)
    "brent_wti_spread",
    # Calendar
    "month",
    "quarter",
...
def _get_features(df: pd.DataFrame) -> list[str]:
    """Return only FEATURES that actually exist in df — no guessing."""
...
def _prepare_Xy(
    ...
    """
    Extract X, y and drop rows where ANY value is NaN/inf.
    get_train_test() already drops lag warm-up NaNs, so this
    is just a safety net for any residual edge cases.
    """
    ...
    if dropped:
        logger.debug(f"  Dropped {dropped} residual NaN rows")
    X_clean = X[valid]
    y_clean = y[valid]
    if len(X_clean) == 0:
        raise ValueError(
            f"No valid rows after NaN drop. "
            f"df shape={df.shape}, features={features}. "
            f"Check that preprocessing column names match FEATURES list."
        )
    logger.info(f"  X shape: {X_clean.shape} | y shape: {y_clean.shape}")
    logger.info(f"  X sample:\n{X_clean.head(3).to_string()}")
